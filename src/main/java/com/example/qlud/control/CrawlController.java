        ...
        if(rawHtmlData != null){
            ...
            for (Element element: elements) {
                ...
                if (scriptContent.contains("window.__GLOBAL_DATA") || scriptContent.contains("window.__INIT_DATA")) {
                    ...
                    electrixOtherProduct += extractDataSection(scriptContent,"\"orderParamModel\":{", "}]}");
...
//                System.out.println("GET request failed");
    ...
    private ProductColorAndSize getElectricOtherProduct(String electricJsonString){
        ...
        JSONObject jsonObject = new JSONObject(json);
        JSONObject orderParamMode = jsonObject.getJSONObject("orderParamModel");
        JSONObject orderParam = orderParamMode.getJSONObject("orderParam");
        int canBookCount = orderParam.getInt("canBookedAmount");
    ...
    private ProductColorAndSize convertStringOtherProductToObject(String otherProduct){
       ...
       if(skuPropsArray.length() > 1){
           ...
           if(colorAndSizeObject.has("prop")){
               productColor.setName(colorAndSizeObject.getString("prop"));
           }else {
               productColor.setName("颜色");
           }
           productSize.setId(colorAndSizeObject.getInt("fid"));
           if(colorAndSizeObject.has("prop")){
               productSize.setName(colorAndSizeObject.getString("prop"));
           }else {
